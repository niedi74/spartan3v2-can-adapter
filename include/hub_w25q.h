...
// v3: + Betriebsstunden (dev_sec/eng_sec/sns_sec) -- war hier auf v2 stehengeblieben,
// waehrend das Hub-Repo (spartan3v2-can-adapter) genau dafuer schon auf v3/v4 ist;
// odo_mm/trip_mm bewusst NICHT uebernommen (Tischtest-Emulator faehrt keine km).
#define HUB_CFG_VERSION 3
...
struct HubConfigBlob {
  ...
  uint8_t  ap_chan;
  char     mdns_host[33];
  // [v3] Betriebsstunden: ueberleben so auch ein erase_flash (NVS weg).
...
void restoreConfigFromW25Q()
{
  ...
  // [v3] Betriebsstunden zurueck ins NVS
  ...
  Serial.printf("Config:      aus W25Q-Backup wiederhergestellt (Profil %d, SSID1 '%s', %.1f Geraete-h)\n",
                b.wifi_prof, b.p1_ssid, static_cast<double>(b.dev_sec) / 3600.0);
}
